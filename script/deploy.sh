...
# docker image pull
echo "> 도커 이미지 pull 받기"
docker pull histour/${server_name}

# green_port로 서버 실행
echo "> ${green_port} 포트로 서버 실행"
echo "> docker run -d --name ${server_name}-${green_port} -p ${green_port}:8080 -e TZ=Asia/Seoul histour/${server_name}"
if [ "$(docker ps -a -q -f name=${server_name}-${green_port})" ]; then
